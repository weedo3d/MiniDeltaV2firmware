...
void GcodeSuite::G33() {
  ...
  #ifdef WTGL_LCD
  wtgl.ShowLog("G33 Auto Calibrate Begin...");
  #endif
  ...
  do { // start iterations
    ...
    if (verbose_level != 0) { // !dry run
      if ((zero_std_dev >= test_precision && iterations > force_iterations) || zero_std_dev <= calibration_precision) { // end iterations
        ...
        #ifdef WTGL_LCD
        wtgl.ShowLog("Calibration Finish.");
        #endif
        SERIAL_ECHO_SP(32);
        #if HAS_BED_PROBE
        ...
        // ui.set_status(mess);
        #ifdef WTGL_LCD
        wtgl.ShowLog("Calibration setting:");
        #endif
        print_calibration_settings(_endstop_results, _angle_results);
        ...
        // ui.set_status(mess);
        #ifdef WTGL_LCD
        wtgl.ShowLogPair("Iteration: %02i std dev: %.2f", (unsigned int)iterations, zero_std_dev);
        #endif
        if (verbose_level > 1)
          print_calibration_settings(_endstop_results, _angle_results);
      }
